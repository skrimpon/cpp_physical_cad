...
{
	std::thread _shell(start_shell, argc, argv);
	start_gui();
	_shell.join();
	return 0;
}
...
int start_shell(int argc, char * argv[])
{
	...
	int expansionresult;
	rl_attempted_completion_function = command_completion;	// Use custom command completion.
	rl_completion_entry_function = NULL;
	rl_completion_append_character = '\0';
	HIST_ENTRY ** _history_list;							// History list, used to store commands.
	Tcl_FindExecutable(argv[0]);							// TCL Find Executable.
	Tcl_Interp * _interpreter = Tcl_CreateInterp();			// Create TCL interpreter.

	Tcl_CreateCommand(_interpreter, "ls", (Tcl_CmdProc*) Tcl_ls, (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
	Tcl_CreateCommand(_interpreter, "clear", (Tcl_CmdProc*) Tcl_clear, (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
	Tcl_CreateCommand(_interpreter, "less", (Tcl_CmdProc*) Tcl_less, (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
	Tcl_CreateCommand(_interpreter, "pwd", (Tcl_CmdProc*) Tcl_pwd, (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
	Tcl_CreateCommand(_interpreter, "vim", (Tcl_CmdProc*) Tcl_vim, (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
	Tcl_CreateCommand(_interpreter, "subl", (Tcl_CmdProc*) Tcl_subl, (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
	Tcl_CreateCommand(_interpreter, "start_gui", (Tcl_CmdProc*) Tcl_start_gui, (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
	
	using_history();
	
	print_msg();
	
	while ((buf = readline("\x1B[38;5;9m→\x1B[0m "))!= NULL)
	{
		if (strcmp(buf, "") == 0)
		{
			continue;
		}
		std::stringstream ss;									// help us seperate the arguments.
		std::string cmd;										// argv[0] of each command.
